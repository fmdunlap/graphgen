...
func main() {
    ...
    parser := graphparse.NewParser()

    graph, err := parser.Parse(string(fileBytes))
    if err != nil {
        panic(err)
    }
    dijkstra(graph, graphparse.NodeId(os.Args[2]), graphparse.NodeId(os.Args[3]))
}

func dijkstra(g *graphparse.Graph, source, target graphparse.NodeId) {
    ...
    queue := []*graphparse.Node{sourceNode}
